...
import { calculateDistanceFromHillsideMall, calculateDeliveryFee, calculateWoodBinsCost } from "../utils/deliveryCalculations.js";
/**
 * Check if vessel has available inventory for the date range
 * Returns number of available units
 */
async function checkVesselAvailability(vesselId, startDate, endDate, excludeBookingId) {
...
export const updateBooking = async (req, res) => {
    try {
        ...
        if (startDate && endDate) {
            ...
            const availability = await checkVesselAvailability(vessel._id.toString(), newStartDate, newEndDate, id // Exclude current booking from conflict check
            );
...
export const extendRental = async (req, res) => {
    try {
        ...
        const availability = await checkVesselAvailability(vessel._id.toString(), new Date(booking.startTime), extendedEndDate, id // Exclude current booking
        );
        if (availability.available === 0) {
            return res.status(409).json({
...
//# sourceMappingURL=adminBookingController.js.map