 ...
 */
export const getTripCapacity = async (tripId) => {
 ...
 */
export const updateTripSeatsBasedOnVessel = async (tripId) => {
    const trip = await Trip.findById(tripId).populate("vessel");
    if (!trip || !trip.vessel) {
        throw new Error("Trip or associated vessel not found");
    }
 ...
 */
export const getTripWithCapacity = async (tripId) => {
    ...
    const vessel = trip.vessel;
    const tripWithCapacity = trip.toObject();
    tripWithCapacity.capacity = vessel?.capacity || 8;
    return tripWithCapacity;
};
//# sourceMappingURL=tripUtils.js.map